...
def solve_part2(puzzle_input: str) -> str:
    grid = parse_grid(puzzle_input)
    first = unwrap(next_down(grid, grid.start))
    index: SickIndex = {}
    total = sick_index(first, grid, index)
...
type SickIndex = Dict[Coord, int]


def sick_index(coord: Coord, grid: Grid, cache: SickIndex) -> int:
    if coord in cache:
        return cache[coord]

    child_coords = get_child_coords(coord, grid)
    children_timelines = sum(sick_index(c, grid, cache) for c in child_coords)

    timelines = children_timelines + 2 - len(child_coords)
    cache[coord] = timelines

    return timelines


def get_child_coords(coord: Coord, grid: Grid) -> set[Coord]:
    return {
        c
        for c in {
